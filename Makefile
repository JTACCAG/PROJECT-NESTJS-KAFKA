...
TAG = latest
IMAGE_NAME = yapeapp.azurecr.io/
CONTAINER_NAME = yapeapp.azurecr.io
...
up: ## Up the docker containers, use me with: make up target=api
	...
	docker network create --driver bridge yapeapp || true
...
cli: ## Execute command in the docker container
	...
	docker run --rm -ti --network yapeapp --env-file .env api_commands yarn start ${cmd}
